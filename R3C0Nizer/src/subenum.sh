...
echo "${yellow} ---------------------------------- xxxxxxxx ---------------------------------- ${reset}"
echo " "
if [ -f ~/go/bin/github-subdomains ]
then
  echo "${magenta} [+] Running github-subdomains for subdomain enumeration${reset}"
    github-subdomains -d $DOM -o ~/reconizer/$DOM/Subdomains/github-subdomains.txt
else
  echo "${blue} [+] Installing github-subdomains don't forget to setup token manually. ${reset}"
  go get -u github.com/gwen001/github-subdomains
  echo "${magenta} [+] Running github-subdomains for subdomain enumeration${reset}"
  github-subdomains -d $DOM -o ~/reconizer/$DOM/Subdomains/github-subdomains.txt
fi
echo " "
echo "${blue} [+] Succesfully saved as github-subdomains.txt  ${reset}"
echo " "
